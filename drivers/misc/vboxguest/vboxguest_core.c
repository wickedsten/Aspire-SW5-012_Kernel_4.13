 ...
 */
static void vbg_balloon_close(struct vbg_dev *gdev, struct vbg_session *session)
{
	int i, rc;
...
int vbg_core_init(struct vbg_dev *gdev, u32 fixed_events)
{
	...
	if (!gdev->mem_balloon.change_req || !gdev->cancel_req ||
	    !gdev->ack_events_req || !gdev->mouse_status_req)
		goto err_free_reqs;
 ...
 */
static int vbg_ioctl_check_memory_balloon(struct vbg_dev *gdev,
					  struct vbg_session *session,
		   ...
		   size_t data_size, size_t *data_size_ret)
{
	...
	*data_size_ret = data_size;
	...
	case VBOXGUEST_IOCTL_CHECK_BALLOON:
		return vbg_ioctl_check_memory_balloon(gdev, session, data);
